...
router.get('/training-records', authenticateToken, async (req, res) => {
  try {
    ...
    const todayCompletionRate = todayTotalMinutes > 0 ? Math.round((todayTotalMinutes / targetMinutesPerDay) * 100) : 0;
...
router.post('/training-records', authenticateToken, async (req, res) => {
  try {
    const userId = req.user.id;
    const { video_id, daily_plan_id, daily_plan_item_id, start_time, end_time, actual_duration_seconds, completed, source } = req.body;
    ...
    const formattedStartTime = new Date(start_time).toISOString().replace('T', ' ').substring(0, 19);
    const formattedEndTime = end_time ? new Date(end_time).toISOString().replace('T', ' ').substring(0, 19) : null;
    ...
    const [result] = await pool.execute(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `, [userId, daily_plan_id || null, daily_plan_item_id || null, video_id, formattedStartTime, formattedEndTime, actual_duration_seconds || 0, completed ? 1 : 0, source]);
